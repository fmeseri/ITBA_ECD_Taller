...
joined_df = (
    df_stocks.withColumnRenamed('dateTime', "full_date")
...
window20 = (
    Window.partitionBy(F.col('symbol')).orderBy(F.col("full_date")).rowsBetween(-20, 0)
)
window50 = (
    Window.partitionBy(F.col('symbol')).orderBy(F.col("full_date")).rowsBetween(-50, 0)
)
window100 = (
    Window.partitionBy(F.col('symbol')).orderBy(F.col("full_date")).rowsBetween(-100, 0)
)

# // Calculate the moving average
stocks_moving_avg_df = (
    joined_df.withColumn("ma20", F.avg("close").over(window20))
...
# Write to Parquet
stocks_moving_avg_df.write.mode('overwrite').partitionBy(
    "year", "month", "day"
).parquet(output_dir)

# Write to Postgres
stocks_moving_avg_df.drop("year", "month", "day").write.format("jdbc").option(
    "url", "jdbc:postgresql://postgres/workshop"
).option("dbtable", "workshop.stocks").option("user", "workshop").option(
    "password", "w0rkzh0p"
).option(
